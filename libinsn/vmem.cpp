...
vmem::vmem(const std::vector<vsegment> segments, int perm) :
    _segments(segments),
    _segNum(0)
{
    if (perm) {
...
vmem::vmem(const vmem& copy, loc_t pos, int perm) :
...
loc_t vmem::memstr(const char *little){
    ...
    retcustomerror(out_of_range,"memstr failed to find \"%s\"",little);
}
