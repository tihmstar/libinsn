...
insn vsegment::operator+(int i){
    ...
    retcustomassure(out_of_range, _size >= 4 && _size >= 4 && _curpos + 4*i < _size-4, "overflow");
...
insn vsegment::operator++(){
    retcustomassure(out_of_range, _size >= 4 && _curpos + 4 < _size-4, "overflow");
    _curpos+=4;
    return getinsn();
}
...
vsegment &vsegment::operator+=(int i){
    if (i<0) return this->operator-=(-i);
    retcustomassure(out_of_range, _size >= 4 && _curpos + 4*i < _size-4, "overflow");
    _curpos+=4*i;
    return *this;
}
...
vsegment &vsegment::operator=(loc_t p){
    if (p == 0){
        ...
        retcustomassure(out_of_range, _size >= 4 && newPos < _size-4, "underflow");
        _curpos = newPos;
    }
    return *this;
}
